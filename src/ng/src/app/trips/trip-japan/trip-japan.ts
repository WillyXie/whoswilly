...
@Component({
  selector: 'app-trip-japan',
  imports: [CommonModule],
  templateUrl: './trip-japan.html',
  styleUrl: './trip-japan.css'
})
export class TripJapan implements AfterViewInit, OnDestroy {
  private map: any;
  private L: any;
  ...
  layers = [
    ...
    { name: '⛩️ Kyoto',   id: '9Bo6_d6rppM' },
    { name: '🌺 Okinawa', id: 'F4bd4II5JAg' }
  ];
  ...
  private async initMap() {
    try {
      ...
      // Initialize map
      this.map = L.map('map').setView([36.2048, 138.2529], 6); // Centered over Japan
